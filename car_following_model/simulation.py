...
def initialize():
    ...
    for i in range(0, total_vehicles):
        ...
        if i == 0:
            veh = Vehicle(i + 1, pos, l=l, v=v_init, a=a, b=b, v_max=v_max_lead, g_min=my_g_min, tau=my_tau,
                          stop_x=stop_location, model=my_model)
        else:
            veh = Vehicle(i + 1, pos, l=l, v=v_init, a=a, b=b, v_max=v_max, g_min=my_g_min, tau=my_tau,
                          stop_x=stop_location, model=my_model)
...
def run_simulation(vehicles, dt, total_time):
    ...
    for step in tqdm(range(1, int(total_time / dt), 1)):
        ...
        if i == 0 and i < sz and vehicles[i].get_position() >= sensor_loc and vehicles[i + 1].get_speed() > 0:
            theta0 = vehicles[i + 1].tau + float(vehicles[i + 1].g_min + vehicles[i + 1].l) / vehicles[
                i].get_max_speed()
        ...
        if i > 0 and i < sz - 1 and vehicles[i].get_position() >= sensor_loc:
            theta0 = vehicles[i + 1].tau + float(vehicles[i].g_min + vehicles[i + 1].l) / vehicles[
                i + 1].get_max_speed()
            theta = vehicles[i + 1].get_headway()
    ...
    plt.figure()
    plt.plot([vehicles[i].time[0], vehicles[i].time[-1]], [vehicles[i].get_max_speed(), vehicles[i].get_max_speed()],
             'r')
    for j in vehicles_to_plot:
