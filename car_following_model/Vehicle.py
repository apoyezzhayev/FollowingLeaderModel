...
class Vehicle:
    ...
    def step_leader(self, x_l, v_l, dt=1):
        ...
        self.gap = x_l - self.x - self.l  # Car length
    ...
    def step_idm(self, x_l, v_l, dt=1):
        ...
        gap_desired = self.g_min + np.max(
            [0, self.v * self.tau + self.v * (self.v - v_l) / (2 * np.sqrt(self.a * (self.b - 0)))])
    ...
    def step_iidm(self, x_l, v_l, dt=1):
        ...
        gap = x_l - self.x - self.l
        gap_desired = self.g_min + np.max(
            [0, self.v * self.tau + self.v * (self.v - v_l) / (2 * np.sqrt(self.a * self.b))])
